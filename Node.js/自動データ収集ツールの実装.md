...
　
//57 try-catch
- エラーがあっても後続処理を止めたくない時に使用
- tryにエラーが起きそうでも実行したい処理
- catchにエラーが起きた時の処理
```javascript
try {
  // エラーが起きる可能性がある処理
...
//61 .envファイルと読み込み
- 設定値を記述するのが.envファイルの役割
- `process.env.環境変数名` で取得する
  
```node
// dotenvで.envファイルから値を読み込むことが出来るようにしている（標準では読み込めない）
import env from "dotenv";
env.config();

// process.envの中に.envファイルの中身が展開されている
// process.env.環境変数名で実際の値を取得する
